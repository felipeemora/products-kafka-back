package com.productskakfa.shopexito.config;
...
public class SpringConfig {
    ...
    MapReactiveUserDetailsService usersDetails() throws Exception {
        List<UserDetails> users = List.of(
            User.withUsername("levi").password("{noop}pass1").roles("USER").build(),
            User.withUsername("doe").password("{noop}pass2").roles("USER").build(),
